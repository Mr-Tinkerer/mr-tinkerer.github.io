...
for md_article in "${mds_to_convert[@]}"; do
    ...
    backlinks=$(grep -oP '\[\[\K[^\]]*\|[^\]]*(?=\]\])' index.md) || true

    if [[-n "$backlinks"]]; then
        echo "$backlinks" | while IFS= read -r line; do
        ...
        echo "No backlinks found for $md_article."
    fi
...
while [[ "$valid_input" != true ]]; do
    ...
    if [[ "$choice" == "Y" || "$choice" == "y" ]]; then
        pkill hugo
        valid_input=true
    #kill the blog, stop the script, and delete the article
    elif [[ "$choice" == "N" || "$choice" == "n" ]]; then
        pkill hugo
        rm -r "$ARTICLE_PATH"
        echo "Deleted $md_article to prevent any errors when rerunning the script"
        exit 0
